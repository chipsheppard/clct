 ...
 */
function clct_dont_update_theme( $r, $url ) {
...
add_filter( 'http_request_args', 'clct_dont_update_theme', 5, 2 );

/**
 * Header Meta Tags
 */
function clct_header_meta_tags() {
	echo '<meta charset="' . esc_html( get_bloginfo( 'charset' ) ) . '">';
	echo '<meta name="viewport" content="width=device-width, initial-scale=1">';
	echo '<link rel="profile" href="http://gmpg.org/xfn/11">';
	echo '<link rel="pingback" href="' . esc_url( get_bloginfo( 'pingback_url' ) ) . '">';
}
add_action( 'wp_head', 'clct_header_meta_tags' );
 ...
 */
function clct_remove_recent_comments_style() {
	global $wp_widget_factory;
	remove_action( 'wp_head', array( $wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style' ) );
}
add_action( 'widgets_init', 'clct_remove_recent_comments_style' );
...
function wrap_archive_title_part( $title ) {
	if ( is_category() ) {
		...
		$title = __( '<div>written by:</div><span class="vcard">', 'clct' ) . get_the_author() . '</span>';
	} elseif ( is_year() ) {
		$title = __( '<div>archive by year:</div>', 'clct' ) . get_the_date( 'Y' );
	} elseif ( is_month() ) {
		$title = __( '<div>archive by month:</div>', 'clct' ) . get_the_date( 'F Y' );
	} elseif ( is_day() ) {
		$title = __( '<div>archive by day:</div>', 'clct' ) . get_the_date( 'F j, Y' );
 ...
 */
function clct_custom_excerpt_length( $length ) {
 ...
 */
function clct_search_button( $text ) {
	$text = str_replace( 'value="Search"', 'value="go"', $text );
	return $text;
}
add_filter( 'get_search_form', 'clct_search_button' );
